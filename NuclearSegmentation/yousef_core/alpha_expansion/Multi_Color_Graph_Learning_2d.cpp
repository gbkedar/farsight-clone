...
{  
    ...
    for(int i=0; i<max_lab; i++)
	{	        
		for(int j=0; j<(int)MAP2[i].size() ; j++)
		{
            NG1 = MAP2[i][j];
			//for each jth neighbor of the ith cell
            for(int k=0; k<(int)MAP2[NG1-1].size() ; k++)
            {
    ...
    for(int i=0; i<r; i++)
    {
        ...
        for(int j=0; j<c; j++)
        {
            ...
            else
            {
                ...
                for (int k = 0; k < (int)MAP[(int)val].size(); k++)
                {
                    V = MAP[(int)val][k]-1;
                    C = ColorOut[V];
			...
			for(int cc=0; cc<ncolors+1; cc++)//[(x+y*width)*#labels + l]				
				out[(((unsigned)j) + ((unsigned)i) * ((unsigned)c)) * (((unsigned)ncolors) + 1) + (unsigned)cc]
							= (float) std::min(-log(Pr[cc]), 100.0);     //I need to double check this line!!           
        }
    }
    for(int i=0; i<max_lab; i++)
    {
